...
export const getGuestOfAnEvent = async (
  req: Request,
  res: Response,
  next: NextFunction
  ...
  if (!eventId) throw new AppError("Query(eventId) not found", 400);
  ...
  const data = await FriendRequestModel.aggregate([
    {
      $match: {
        $or: [
          { sender: new mongoose.Types.ObjectId(eventUserId) },
          { receiver: new mongoose.Types.ObjectId(eventUserId) },
        ],
      },
    },
    {
      $project: {
        ...
        otherUserIds: {
          $cond: {
            if: { $eq: ["$sender", new mongoose.Types.ObjectId(eventUserId)] },
            then: "$receiver",
            else: "$sender",
          },
        },
  ...
  const connectedUsers = await FriendModel.aggregate([
    ...
    },
    {
      $project: {
        ...
        otherUserIds: {
          $cond: {
            if: { $eq: ["$user1", new mongoose.Types.ObjectId(eventUserId)] },
            then: "$user2",
            else: "$user1",
          },
        },
  ...
  const connectedUsersIds =
    connectedUsers && connectedUsers.length > 0
      ? connectedUsers[0].otherUserIds
      : [];
  const pendingRequestUserIds =
    data && data.length > 0 ? data[0].otherUserIds : [];

  const knownUserIdsArray = [...pendingRequestUserIds, ...connectedUsersIds];
  ...
  const currentEventUser = await EventUserModel.findOne({
    _id: new mongoose.Types.ObjectId(String(eventUserId)),
    eventId: new mongoose.Types.ObjectId(String(eventId)),
  }).select(
    "profession position company instituteName courseName industry lookingFor"
  );
  ...
  const eventGuests = await EventUserModel.aggregate([
    ...
    },
    {
      $addFields: {
        ...
        userData: {
          $mergeObjects: [
            {
              _id: "$_id", // Use EventUser ID
              userId: "$userId", // Keep reference to main User ID
              name: "$name", // Use name from EventUser
              email: "$email", // Use email from EventUser
    ...
    },
    {
      $match: {
        $and: [
          { _id: { $ne: new mongoose.Types.ObjectId(String(eventUserId)) } }, // Exclude current EventUser
          { _id: { $not: { $in: knownUserIds } } }, // Check against EventUser IDs for friendship status
          {
            $expr: {
              $cond: {
                ...
                then: {
                  $gt: [
                    {
                      $size: {
                        $setIntersection: ["$lookingFor", selectedInterest],
                      },
                    },
                    0,
                  ],
...
// past → khatam ho gaya
export const getAllEventsOfUser = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<Response | void> => {
  ...
  const events = await EventUserModel.aggregate([
    {
      $match: { _id: new mongoose.Types.ObjectId(String(eventUserId)) },
    },
    {
      $lookup: {
        from: "events",
      ...
      $project: {
        ...
        status: {
          $cond: {
            if: {
              $and: [
                { $gte: ["$events.endDate", currentDate] },
                { $lte: ["$events.startDate", currentDate] },
              ],
            },
            then: "ongoing",
            else: {
    ...
    },
    {
      $group: {
        ...
        upComing: {
          $push: {
            $cond: [{ $gt: ["$startDate", currentDate] }, "$$ROOT", null],
          },
        },
        past: {
          $push: {
            $cond: [{ $lt: ["$endDate", currentDate] }, "$$ROOT", null],
          },
        },
      },
    },
...
export const getAttendiesRole = async (
  ...
  if (!eventId) throw new AppError("Query not found", 400);
  ...
  if (!attendeeRoles.length) throw new AppError("Roles not found", 404);
...
export const searchGuestInEvents = async (
  ...
  const parsedLimit = Number.isNaN(parseInt(String(limit)))
    ? 10
  ...
  const eventGuests = await EventUserModel.aggregate([
      ...
      $addFields: {
        userData: {
          $mergeObjects: [
            ...
            {
              isConnected: {
                $in: ["$_id", knownUserIds], // Use EventUser ID for connection check
              },
              lookingForMatchedCount: {
                $size: {
                  $setIntersection: [
              ...
              isAnyMatch: name
                ? {
                    $or: [
                      {
                        $regexMatch: {
                          input: "$name", // Use name from EventUser directly
                          regex: name,
                          options: "i",
                        },
                      },
    ...
    ...(cursor
      ? [
          {
            $match: {
              _id: { $gt: new mongoose.Types.ObjectId(String(cursor)) }, // Use EventUser ID for cursor
            },
          },
        ]
      : []),
